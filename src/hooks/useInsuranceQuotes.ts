      ...
      try {
        ...
        if (data && data.result) {
          return Object.entries(data.result).map(([key, value]: [string, any]) => {
            let provider = value.companyName || 'Unknown';
            
            const planName = value.planName || value.planType || key;
            ...
            const benefits = value.covers?.slice(0, 3).map((cover: any) => ({
              icon: "✓",
              text: cover.coverName
            })) || [];
            
            const coveragePoints = value.covers?.slice(3, 5).map((cover: any) => 
              `$ ${cover.coverAmount} ${cover.coverName}`
            ) || [];
            ...
            return {
              ...
              name: planName,
              provider: provider,
              logo: provider.toLowerCase().includes('godigit') 
