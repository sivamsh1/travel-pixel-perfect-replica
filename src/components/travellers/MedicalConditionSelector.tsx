...
const MedicalConditionSelector: React.FC<MedicalConditionSelectorProps> = ({
  ...
  return <div className="mb-8 space-y-4">
      <div>
        ...
        <div className="flex space-x-6">
          <div className="flex items-center space-x-2">
            <Checkbox id="medical-yes" checked={traveller?.hasPreExistingCondition === true} onCheckedChange={() => updateTraveller(travellerIndex, {
          ...
          <div className="flex items-center space-x-2">
            <Checkbox id="medical-no" checked={traveller?.hasPreExistingCondition === false} onCheckedChange={() => updateTraveller(travellerIndex, {
      ...
      {traveller?.hasPreExistingCondition && <div className="space-y-2">
          
          <Select value={traveller.medicalCondition} onValueChange={value => updateTraveller(travellerIndex, {
        medicalCondition: value
            ...
            <SelectContent>
              {medicalConditions.map(condition => <SelectItem key={condition} value={condition}>
                  {condition}
                </SelectItem>)}
            </SelectContent>
          </Select>
